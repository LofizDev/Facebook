...
import Resize_TextEffect from '../post_resize/Resize_TextEffect';
...
function CardBody({ post }) {
    ...
    return (
        <div className={classes.contentPost}>
            {post.images.length !== 0 || post.optionTextEffect === '' && (
                <div style={{ padding: '0 16px 4px', textAlign: 'justify' }}>
            ...
            <Link to={`/post/${post._id}`}>
                <div style={{ display: 'flex', flexWrap: 'wrap', cursor: 'pointer' }}>
                    ...
                    {post.images.length === 0 && post.optionTextEffect !== '' && <Resize_TextEffect post={post} />}
                </div>
            </Link>
        </div>
